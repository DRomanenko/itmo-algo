...
class SuffixArray {
 private:
    ...
    int get_min(vector<vector<int>> &sparse, int l, int r) {
        if (l > r)
            swap(l, r);
 ...
 public:
    ...
    void count_diff_substr_for_all_prefix() {
        precalc_lcp();
 
        vector<vector<int>> sparse(1);
        sparse[0] = lcp;
        for (int i = 1; (1 << i) <= (int)data.size(); ++i) {
            sparse.resize(sparse.size() + 1);
            for (int q = 0; q <= (int)data.size() - (1 << i); ++q) {
                sparse[i].push_back(std::min(sparse[i - 1][q], sparse[i - 1][q + (1 << (i - 1))]));
            }
        }
        ...
        used.insert(-1);
        vector<ll> ans;
        vector<int> inv_suff_arr(data.size());
        ans.emplace_back(1);
        for (size_t i = 0; i < data.size(); i++)
            inv_suff_arr[suff_arr[i]] = i;
        used.insert(inv_suff_arr[data.size() - 1]);
        ll result = 1;
        for (int i = (int)data.size() - 2; i >= 0; --i) {
            int help = 0;
            auto insert_pos = used.insert(inv_suff_arr[i]);
            if (++insert_pos.first != used.end())
                help = max(help, get_min(sparse, inv_suff_arr[i], *insert_pos.first - 1));
            --insert_pos.first;
            --insert_pos.first;
            if (*insert_pos.first != -1)
                help = max(help, get_min(sparse, *insert_pos.first, inv_suff_arr[i] - 1));
            result += (ll) data.size() - i - help;
...
signed main() {
    ...
    suffixArray->count_diff_substr_for_all_prefix();
    delete suffixArray;
    return 0;
}