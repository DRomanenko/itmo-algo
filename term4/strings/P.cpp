...
vector<vector<size_t>> precalc_all_lcp(const string &s) {
    string work_s = "#" + s;
    vector<vector<size_t>> all_lcp(work_s.size(), vector<size_t>(work_s.size(), 0));
    for (size_t l = 1; l < work_s.size(); ++l)
        for (size_t r = 1; r < work_s.size(); ++r)
            if (l != r && work_s[l] == work_s[r])
                all_lcp[l][r] = all_lcp[l - 1][r - 1] + 1;
    return all_lcp;
}
 
vector<vector<size_t>> precalc_all_count_for_substr(const vector<vector<size_t>> &lcp) {
    vector<vector<size_t>> ans(lcp.size(), vector<size_t>(lcp[0].size(), 0));
    for (size_t l = 1; l < lcp.size(); ++l) {
        size_t maxx = 0;
        for (int r = (int) l - 1; r > 0; --r) {
            if (lcp[l][r] > maxx) {
                for (size_t t = maxx + 1; t <= lcp[l][r]; ++t)
                    ++ans[r - t + 1][l];
                maxx = (size_t) lcp[l][r];
            }
        }
    }
    for (int l = (int) lcp.size() - 1; l > 0; --l)
        for (size_t r = 1; r < lcp.size() - 1; ++r)
            ans[l][r + 1] += ans[l][r];
    for (int l = (int) lcp.size() - 1; l > 1; --l)
        for (size_t r = 1; r < lcp.size(); ++r)
            ans[l - 1][r] += ans[l][r];
    return ans;
}
...
int main() {
    ...
    ll ans = c % MOD_FOR_COUNT;
    for (size_t i = 1; i < fs.size(); ++i) {
        if (fs[i] == 0) {
            ll curr_f = fs[i - 1], curr_c = c - 1;
            while (curr_f > 0) {
                curr_c = (fs[(size_t) curr_f] ? curr_c : curr_c - 1);
                curr_f = fs[i - 1];
            }
            ans = (ans * curr_c) % MOD_FOR_COUNT;
        }
    }
    cout << ans << '\n';
    return 0;
