    ...
    class BlockingFlowBase : public MaxFlowFinder {
     protected:
        ...
        NNetwork::Path findPath() {
            ...
            if (used[v]) {
                while (!Q.empty() && (*(Q.begin())).first.from != v)
                    Q.pop_front();
            }
            ...
            for (auto curr_edge : Q) {
                ...
                ans.path.push_back(curr_edge.second);
            }
            ans.flow = min_flow;
            for (auto curr_edge : Q) {
...
int main() {
    ...
    size_t n, m;
    cin >> n >> m;
    auto *net = new NNetwork::Network(100 * n + 2, 0, n + 1);
    long long max_capacity = -1;
    for (size_t i = 0; i < m; ++i) {
        size_t a, b;
        long long low, capacity;
        cin >> a >> b >> low >> capacity;
        max_capacity = max(max_capacity, capacity);
        net->addEdge(0, b, low, i);
        net->addEdge(a, b, capacity - low, i);
        net->addEdge(a, n + 1, low, i);
    }
    net->setScale(1LL << ((long long) log2((double) max_capacity)));
    auto solver = NMaxFlow::DinicAlgorithm();
    solver.setNetwork(net);
    solver.buildMaxFlow();
    if (solver.findCircle()) {
        cout << "YES\n";
        for (size_t i = 0; i < m; ++i) {
            cout << net->getCircle()[i] << '\n';
        }
    } else {
        cout << "NO\n";
    }
